...
const homePage = async (req, res) => {
    try {
        const limitNum = 5;
        const categories = await categorySchema.find({}).limit(limitNum);
...
const showRandom = async (req, res) => {
    try {

        const totalRecipe = await recipe.countDocuments()
        const random = Math.floor(Math.random() * totalRecipe)
        const Recipe = await recipe.findOne().skip(random)
        res.render('recipe', { title: 'Cooking Blog  RANDOM Recipe', Recipe });
    } catch (err) {
        res.status(500).send({ message: err.message || "error occured" })
    }
}
const searchRecipe = async (req, res) => {
    try {
        ...
        const Recipe = await recipe.find({ $text: { $search: search } })
